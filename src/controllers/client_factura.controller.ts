...
import {
  createClientInteractor,
  updateClientInteractor,
  deleteClientInteractor,
} from "../core/interactor/client_facturas";
...
export const createClient = async (
  ...
  try {
    ...
    const cliente = new ClientFactura();
...
export const updateClient = async (
  ...
  try {
    ...
    const cliente = await getRepository(ClientFactura).findOne(req.params.id);
    if (!cliente) {
...
export const deleteClient = async (
  ...
  try {
    const clientById = await getRepository(ClientFactura).findOne(req.params.id);
    if (!clientById) {
...
export const listClient = async (
  ...
  try {
    ...
    let where:
      | string
      | ObjectLiteral
      | FindConditions<ClientFactura>
      | FindConditions<ClientFactura>[]
      | undefined = { isActive: true };

    const [result, count] = await getRepository(ClientFactura).findAndCount({
      take,
      skip: skip * take,
      where: [
        {
